      ...
      try {
        ...
        allPostsThisWeek.forEach((post) => {
          ...
          if (!authorMap.has(authorId)) {
            // Tạo slug: tên + id rút gọn để tránh trùng
            const shortId = authorId.slice(-6) // 6 ký tự cuối của UID
            const slug = createSlug(`${post.authorName}-${shortId}`)
            authorMap.set(authorId, {
              authorId,
              authorName: post.authorName || 'Unknown',
              authorPhotoURL: post.authorPhotoURL || '',
              postCount: 0,
              slug // lưu slug vào object
            })
          }
            ...
            {loading ? (
              ...
              <div className='space-y-5'>
                {topAuthors.map((author) => {
                  return (
                    <Link key={author.authorId} to={`/author/${author.authorId}`} className='block'>

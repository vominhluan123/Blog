          ...
          <div className='absolute inset-0'>
            {featuredPost.image ? (
              <div className='md:w-1/3 overflow-hidden rounded-lg'>
                {' '}
                {/* Thêm overflow-hidden để crop đẹp */}
                <div className='aspect-4/3 w-full'>
                  {' '}
                  {/* Khóa tỷ lệ 4:3 - bạn có thể đổi thành aspect-video (16/9), aspect-[3/2], aspect-square... */}
                  <img
                    src={featuredPost.image}
                    alt={featuredPost.title}
                    className='w-full h-full object-cover transition-transform duration-300 hover:scale-105' // object-cover crop fill đẹp + zoom nhẹ khi hover
                    loading='lazy' // Tối ưu load
                  />
                </div>
              </div>
            ) : (
            ...
            {/* Overlay tối hơn để text nổi */}
            <div className='absolute inset-0 bg-linear-to-t from-black/70 via-black/40 to-transparent' />
          </div>

          {/* Nội dung */}
          <div className='relative px-6 py-16 md:px-12 md:py-24 lg:px-16 lg:py-32 max-w-5xl mx-auto'>
            {/* Badge Nổi bật */}
            <div className='mb-6 inline-flex items-center rounded-full bg-primary/90 px-4 py-1.5 text-sm font-medium text-primary-foreground shadow-md'>
              <span className='mr-1.5'>⭐</span> Bài viết nổi bật
            </div>

            <h2 className='text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-white leading-tight drop-shadow-lg'>
              {featuredPost.title}
            </h2>

            <p className='mt-6 text-lg md:text-xl text-white/90 line-clamp-4 max-w-3xl'>
            ...
            <div className='mt-8 flex flex-col sm:flex-row sm:items-center gap-6'>
              ...
              <div className='flex items-center gap-4 text-white/90'>
                <Avatar className='h-12 w-12 border-2 border-white/50 shadow-md'>

...
const Header = () => {
  ...
  // ── Search ──
  const [searchQuery, setSearchQuery] = useState('') // chữ người dùng đang gõ
  const [searchResults, setSearchResults] = useState<Array<{ id: string; title: string }>>([]) // danh sách bài tìm được
  const [isSearching, setIsSearching] = useState(false) // đang tải không?
  const [isFocused, setIsFocused] = useState(false) // dùng để biết "click ra ngoài"
  const searchRef = useRef<HTMLDivElement>(null)
  // Click ra ngoài thì ẩn kết quả tìm kiếm
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (searchRef.current && !searchRef.current.contains(e.target as Node)) {
        // Delay nhỏ để onClick của Button chạy xong trước khi ẩn
        setTimeout(() => {
          setIsFocused(false)
        }, 0)
      }
    ...
    document.addEventListener('click', handleClickOutside) // dùng 'click' thay 'mousedown'
      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16'>
        ...
        <div className='flex items-center gap-4 relative' ref={searchRef}>
          <div className='relative hidden md:block'>
            <Search className='absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-foreground/80 pointer-events-none z-10' />
            <Input
              id='search-input' // ← fix cảnh báo xanh
              name='search'
              ...
              autoCorrect='off'
              autoCapitalize='off'
              className={cn(searchInputClass, 'pl-10 w-72 text-sm rounded-xl')}
            />

            {/* Dropdown chỉ hiện khi đang focus */}
            {isFocused && (
              <div className='absolute top-full left-0 mt-2 w-full bg-popover border border-border/50 rounded-xl shadow-xl z-50 max-h-[320px] overflow-hidden'>
                {/* Header nhỏ nếu cần (tùy chọn) */}
                {/* <div className="px-4 py-2 text-xs text-muted-foreground font-medium border-b">Kết quả tìm kiếm</div> */}
                  ...
                  <div className='max-h-[280px] overflow-y-auto'>
                    {searchResults.map((result) => (
                      <Button
                        key={result.id}
                        variant='ghost'
                        className='w-full h-auto justify-start px-4 py-2.5 text-sm hover:bg-accent/70 transition-colors flex items-center gap-3 rounded-none'
                        onClick={(e) => {
                          e.preventDefault()
                          e.stopPropagation()
                      ...
                      >
                        ...
                        <span className='line-clamp-1 flex-1 text-left'>{result.title}</span>
                      </Button>
                    ))}
                  </div>
                ) : searchQuery.trim().length >= 2 ? (
                  <div className='px-4 py-6 text-center text-sm text-muted-foreground'>Không tìm thấy bài viết</div>
                ) : (
                  <div className='px-4 py-6 text-center text-sm text-muted-foreground'>Gõ để tìm kiếm bài viết...</div>
                )}
              </div>
            )}
          </div>

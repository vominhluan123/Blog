        ...
        {posts ? (
          <div className='mb-10 rounded-2xl bg-linear-to-r from-primary to-primary/80 p-8 text-primary-foreground overflow-hidden shadow-xl'>
            <div className='max-w-3xl'>
              <h2 className='text-h2 font-extrabold leading-tight'>{posts.title}</h2>
              <p className='mt-4 text-primary-foreground/90 text-body line-clamp-3'>
                {posts.content.substring(0, 200)}...
              </p>
              <div className='mt-6 flex items-center gap-4'>
                <Button variant='secondary' size='lg' asChild>
                  <Link to={`/post/${posts.id}`}>Đọc ngay</Link>
                </Button>
                <div className='text-sm opacity-80 flex items-center gap-2'>
                  <Avatar className='h-8 w-8'>
                    <AvatarImage src={posts.authorPhotoURL || 'https://github.com/shadcn.png'} />
                    <AvatarFallback>{posts.authorName?.[0] || 'A'}</AvatarFallback>
                  </Avatar>
                  <span>
                    {posts.authorName} • {posts.createdAt.toDate().toLocaleDateString()}
                  </span>
                </div>
              </div>
            </div>

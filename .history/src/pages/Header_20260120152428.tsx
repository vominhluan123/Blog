...
const Header = () => {
    ...
    <div className="relative w-64 hidden md:block">
      ...
      <Input
        ...
        onChange={(e) => {
          ...
          setOpen(true)           // mở popover khi gõ
        }}
        onFocus={() => setOpen(true)}
        className={cn(
          searchInputClass,
          "cursor-text pl-10"     // thêm padding trái cho icon
        )}
      />
    </div>
  </PopoverTrigger>
  ...
  <PopoverContent
    className="w-[var(--radix-popover-trigger-width)] p-0"  // khớp width với input
    align="start"
    sideOffset={6}
  >
    <Command 
      shouldFilter={false}   // chúng ta filter thủ công bằng Firestore
      className="overflow-visible"
    >

          {/* Dropdown gợi ý */}
          {searchResults.length > 0 && (
            <div className='absolute top-full left-0 md:left-auto md:right-0 mt-2 w-full md:w-80 bg-popover border rounded-xl shadow-xl z-50 max-h-96 overflow-y-auto'>
              {isSearching ? (

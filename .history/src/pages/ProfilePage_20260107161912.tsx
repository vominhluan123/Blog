...
import { useEffect, useState } from 'react' // ← thêm useRef
...
const Profile = () => {
  ...
  useEffect(() => {
    // Tắt scroll restoration mặc định của browser (rất quan trọng!)
    if ('scrollRestoration' in history) {
      history.scrollRestoration = 'manual'
    }

    // Force scroll lên đầu ngay lập tức
    window.scrollTo({
      top: 0,
      left: 0,
      behavior: 'instant' // 'instant' để lên đầu ngay, không animation
      // Nếu muốn mượt một chút thì dùng 'smooth' + delay nhỏ
    })

    // Optional: Delay nhẹ để chắc chắn DOM render xong (thử nếu instant không chạy)
    // setTimeout(() => {

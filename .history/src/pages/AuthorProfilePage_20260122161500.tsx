...
import { Button } from '@/components/ui/button' // nếu muốn thêm nút "Load more"
...
const AuthorProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAuthorAndPosts = async (isInitial = true) => {
      try {
        if (isInitial) {
          setLoading(true)
          setNotFound(false)
        ...
        if (isInitial) {
          ...
          setAuthorInfo({
            ...
            postCount: 0, // sẽ update sau
            uid
          })
        } else if (!authorInfo?.uid) {
          return // tránh lỗi nếu chưa có uid
        } else {
          uid = authorInfo.uid
        }
        ...
        // Update postCount (ước lượng, hoặc dùng aggregation nếu cần chính xác)
        if (isInitial) {
  ...
  }, [loading, loadingMore, hasMore, lastDoc, authorInfo?.uid])
...
function fetchAuthorAndPosts(arg0: boolean) {

 ...
 useEffect(() => {
   ...
   const fetchAuthorData = async () => {
     try {
       ...
       // Chuẩn hóa slug từ URL thành dạng giống displayName gốc (title case hoặc giữ nguyên case nếu cần)
       // Nhưng vì DB lưu "minh Luân" → ta cần convert về dạng gần giống nhất
       // Cách đơn giản: thay - bằng space, rồi capitalize mỗi từ
       const normalizedName = authorId
         .replace(/-/g, ' ') // minh-luan → "minh luan"
         .split(' ')
         .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
         .join(' ') // → "Minh Luan"

       // Nếu bạn biết displayName trong DB thường viết hoa chữ cái đầu → dùng cái này
       // Hoặc nếu muốn chính xác hơn, có thể thử cả 2 cách: "minh Luân" và "Minh Luân"

...
const schema = z
  .object({
    firstname: z.string().min(1, 'Họ không được để trống').max(10, 'Họ tối đa 10 ký tự').trim(),
    lastname: z.string().min(1, 'Tên không được để trống').max(5, 'Tên tối đa 5 ký tự').trim(),
    email: z.string().email('Email không hợp lệ').min(1, 'Email không được để trống').trim(),
    reactselect: z.string().min(1, 'Vui lòng chọn giới tính'),
  ...
  .refine(
    (data) => data.reactselect !== 'other' || ((data.otherGender as string | undefined)?.trim()?.length ?? 0) > 0,
    {
      path: ['otherGender'],
      message: 'Vui lòng nhập giới tính cụ thể'
    }
  )

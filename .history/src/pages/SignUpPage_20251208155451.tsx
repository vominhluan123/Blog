...
import { GoogleAuthProvider, signInWithEmailAndPassword, signInWithPopup, updateProfile, type User } from 'firebase/auth'
...
const SignUpPage = () => {
  ...
  const handleSignUp: SubmitHandler<FormData> = async (e) => {
    try {
      const userCredential = await signInWithEmailAndPassword(auth, e.email, e.password)
      // set displayname
      if (auth.currentUser) {
        await updateProfile(auth.currentUser, { displayName:e.fullname })
        await auth.currentUser.reload()
        const freshUser = { ...auth.currentUser } as User
        setUser(freshUser)
      }
      // lưu vào firebase

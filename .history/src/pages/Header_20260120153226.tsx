          ...
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <div className='relative hidden md:block'>
                ...
                {/* Dùng div làm trigger, nhưng input bên trong sẽ điều khiển open */}
                <Input
                  ...
                  className={cn(searchInputClass, 'pl-10 cursor-text w-64')}
                />
              </div>
            </PopoverTrigger>
            ...
            <PopoverContent
              ...
              align='start'
              sideOffset={8}
              onOpenAutoFocus={(e) => e.preventDefault()} // Ngăn popover focus làm mất focus input
            >
              <Command
                shouldFilter={false}
                className='overflow-visible'
                loop // Cho phép dùng phím lên/xuống vòng lặp
              >
                {/* CommandInput đặt ở đây để popover không đóng khi gõ */}
                <CommandInput
                  placeholder='Tìm bài viết...'
                  value={searchQuery}
                ...
                <CommandList className='max-h-80 overflow-y-auto'>
                  {isSearching ? (
                    ...
                    <CommandGroup heading='Kết quả tìm kiếm' className='p-2'>
                      {searchResults.map((result) => (
                        <CommandItem
                          key={result.id}
                          value={result.id} // Giúp keyboard navigation
                          onSelect={() => {
                            setSearchQuery('')
                            setSearchResults([])
                            setOpen(false)
                    ...
                    <div className='py-6 text-center text-sm text-muted-foreground'>Gõ để tìm kiếm...</div>
                  )}
                </CommandList>
              </Command>
            </PopoverContent>

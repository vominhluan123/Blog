        ...
        <Card key={post.id} className='overflow-hidden hover:shadow-lg transition-shadow relative group'>
          ...
          <CardContent className='p-6 space-y-4'>
            <div className='flex justify-between items-start'>
              ...
              {/* Nút Xem trước (preview modal) - hiện cho mọi trạng thái */}
              <Button variant='ghost' size='icon' className='h-8 w-8' onClick={() => setPreviewPost(post)}>
            ...
            {previewPost && (
              <Dialog open={!!previewPost} onOpenChange={() => setPreviewPost(null)}>
                <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
                  <DialogHeader>
                    <DialogTitle className='text-2xl font-bold'>{previewPost.title}</DialogTitle>
                  </DialogHeader>
                  ...
                  <div className='space-y-6 py-4'>
                    ...
                    <div className='prose prose-lg max-w-none'>
                      <PostContentPreview content={previewPost.content} />{' '}
                      {/* Hoặc dùng QuillPreview nếu muốn full HTML */}
                    </div>

                    <div className='text-sm text-muted-foreground border-t pt-6'>
                      <p>Tác giả: {previewPost.authorName}</p>
                      <p>Ngày gửi: {previewPost.createdAt.toDate().toLocaleString('vi-VN')}</p>
                      {previewPost.rejectReason && (
                        <p className='text-destructive mt-2'>Lý do từ chối: {previewPost.rejectReason}</p>
                      )}
                    </div>

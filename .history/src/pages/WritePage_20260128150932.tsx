...
const WritePage = () => {
  ...
  // Handler upload ảnh khi click nút image trong toolbar
  const handleImageUpload = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (data.secure_url) {
          // Lấy Quill instance và chèn ảnh
          if (quillRef.current) {
            const quill = quillRef.current.getEditor()
            const range = quill.getSelection(true) // vị trí con trỏ hiện tại
  ...
  const modules = {
    toolbar: {
      container: [
        ...
        ['link', 'image'], // nút link và image
        [{ list: 'ordered' }, { list: 'bullet' }],
        ['clean']
      ],
      handlers: {
        image: handleImageUpload // handler upload ảnh thủ công
      }
    }
  }
              ...
              <div className='space-y-2'>
                ...
                >
                  <ReactQuill
                    ...
                    onChange={(html) => setValue('content', html, { shouldValidate: false })}
                    modules={modules}
                    formats={formats}

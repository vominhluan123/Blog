      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16'>
        ...
        <div className='flex items-center gap-4 relative' ref={searchRef}>
          <div className='relative hidden md:block'>
            <Search className='absolute left-3 top-1/2 -translate-y-1/2 h-3 w-3 text-foreground/80 pointer-events-none z-10' />
            <Input
              placeholder='Tìm bài viết...'
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onFocus={() => {
                // Mở dropdown nếu có query hoặc đang có kết quả
                if (searchQuery.trim().length >= 2 || searchResults.length > 0) {
                  setSearchResults(searchResults) // trigger re-render nếu cần
                }
              }}
              className={cn(
                searchInputClass,
                'pl-11 w-64' // tăng padding một chút cho icon to hơn
              )}
            />
            ...
            {(searchResults.length > 0 ||
              ...
              <div
                ...
                onMouseDown={(e) => e.stopPropagation()} // Ngăn click outside ẩn dropdown khi click vào nó
              >
                {isSearching ? (

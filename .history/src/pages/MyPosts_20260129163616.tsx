...
import { collection, query, where, getDocs, getCountFromServer, doc, updateDoc, Timestamp, orderBy } from 'firebase/firestore'
...
const MyPosts = () => {
  ...
  // Fetch count riêng biệt (chính xác, nhanh, không tốn băng thông)
        ...
        <Card key={post.id} className='overflow-hidden hover:shadow-lg transition-shadow relative group'>
          ...
          <CardContent className='p-6 space-y-4'>
            <div className='flex justify-between items-start'>
              ...
              >
                {post.status === 'pending' ? 'Chờ duyệt' : post.status === 'approved' ? 'Đã duyệt' : 'Bị huỷ'}
              </Badge>
            </div>

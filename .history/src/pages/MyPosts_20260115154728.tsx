      ...
      {posts.map((post) => (
        <Card key={post.id} className='overflow-hidden hover:shadow-lg transition-shadow'>
          ...
          <CardContent className='p-6 space-y-4'>
            ...
            <p className='text-sm text-muted-foreground line-clamp-3'>{post.content.substring(0, 150)}...</p>
            ...
            {post.status === PostStatus.REJECTED && (
              <div className='mt-3 p-3 bg-destructive/10 border border-destructive/30 rounded-md text-sm text-destructive'>
                <strong>Lý do từ chối:</strong> {post.rejectReason || 'Không có lý do cụ thể'}
                <p className='mt-1 text-xs text-muted-foreground'>Vui lòng chỉnh sửa và gửi lại bài viết nhé!</p>
              </div>
            )}

            {/* Nếu APPROVED và isFeatured */}
            {post.status === PostStatus.APPROVED && post.isFeatured && (

...
const AdminPosts = () => {
  ...
  const handleLoadMore = async() => {
    ...
    const first = query(collection(db, POSTS_COLLECTION), orderBy('createdAt', 'desc'), limit(1))
    ...
    const next = query(collection(db, POSTS_COLLECTION), orderBy('population'), startAfter(lastVisible), limit(25))
  }
  return (
    <div className='min-h-screen p-8 bg-background'>
      ...
      <div className='mt-10 text-center'>
        <Button onClick={handleLoadMore}>Tải Tiếp</Button>
      </div>
    </div>
  )
}

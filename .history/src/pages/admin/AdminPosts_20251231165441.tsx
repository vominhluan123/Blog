                        ...
                        <div className='space-y-2'>
                          ...
                          <Select onValueChange={setCategory} defaultValue={category}>
                            <SelectTrigger
                              className={cn(
                                formInputClass,
                                'w-full h-10 text-base', // Chiều cao vừa, chữ to hơn
                                'focus:ring-primary/50 focus:ring-offset-background',
                                'dark:focus:ring-primary/70 dark:focus:ring-offset-muted'
                              )}
                            >
                              <SelectValue placeholder='Chọn danh mục...' />
                            </SelectTrigger>
                            <SelectContent
                              className='max-h-[300px] overflow-y-auto z-50' // Giới hạn chiều cao + z-index cao
                              position='popper' // Cố định vị trí, không nhảy
                              sideOffset={4} // Khoảng cách với trigger
                            >
                              {categories.map((cat) => (
                                <SelectItem
                                  key={cat}
                                  value={cat}
                                  className='cursor-pointer !hover:bg-red-500 focus:bg-primary/20 transition-colors'
                                >
                                  {cat}
                                </SelectItem>
                              ))}

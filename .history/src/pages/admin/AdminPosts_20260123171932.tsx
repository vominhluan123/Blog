...
import { type Post, POSTS_COLLECTION, PostStatus, USERS_COLLECTION } from '@/firebase/db'
...
const AdminPosts = () => {
  ...
  // tự add slug cho tất cả user cũ có displayName nhưng chưa có slug.
  ...
  //       const slug = await getOrCreateUniqueSlug(data.displayName, userDoc.id)
  //       await updateDoc(userDoc.ref, { slug })
  //       console.log(`Đã thêm slug "${slug}" cho user ${userDoc.id}`)
  //       updatedCount++
  //     }
  //   }
  //   console.log(`Hoàn tất! Cập nhật ${updatedCount} user.`)
  // }
  // addMissingSlugs()
  async function addAuthorSlugToPosts() {
    const postsSnap = await getDocs(collection(db, POSTS_COLLECTION))
    console.log(`Tổng số bài viết: ${postsSnap.size}`)
    ...
    for (const postDoc of postsSnap.docs) {
      ...
      if (!post.authorId) {
        console.log(`Post ${postDoc.id} thiếu authorId → skip`)
        skipped++
        continue
      }
      if (post.authorSlug) {
        console.log(`Post ${postDoc.id} đã có authorSlug: ${post.authorSlug} → skip`)
        skipped++
        continue
      }

      const userRef = doc(db, USERS_COLLECTION, post.authorId)
      const userSnap = await getDoc(userRef)
      if (userSnap.exists()) {
        const slug = userSnap.data().slug
        if (slug) {
          console.log(`Cập nhật post ${postDoc.id} với slug: ${slug}`)
          batch.update(postDoc.ref, { authorSlug: slug })
          count++
        } else {
          console.log(`User ${post.authorId} không có slug → skip`)
        }
      } else {
        console.log(`Không tìm thấy user cho post ${postDoc.id}, authorId: ${post.authorId}`)
      }

      if (count % 500 === 0 && count > 0) {
        await batch.commit()
        console.log(`Commit batch tại count = ${count}`)
        batch = writeBatch(db)
      }
    }
    ...
    console.log(`Đã cập nhật ${count} bài viết`)
    console.log(`Đã skip ${skipped} bài (thiếu authorId hoặc đã có slug)`)
    console.log(`Tổng post xử lý: ${postsSnap.size}`)
  }
  async function migrateUsersSlug() {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      if (!displayName.trim()) {
        console.log(`User ${userDoc.id} thiếu displayName → skip`)
        continue
      }

      if (data.slug) {
        console.log(`User ${userDoc.id} đã có slug: ${data.slug} → skip`)
        continue
      }

      try {
        const newSlug = await getOrCreateUniqueSlug(displayName, userDoc.id)
        batch.update(userDoc.ref, {
          slug: newSlug,
          updatedAt: new Date() // optional
        })
        console.log(`Tạo slug cho ${userDoc.id}: ${newSlug}`)
        updated++
      } catch (err) {
        console.error(`Lỗi user ${userDoc.id}:`, err)
      }

      // Commit mỗi 400-500 để tránh giới hạn
    ...
    await batch.commit()
    console.log(`Đã cập nhật slug cho ${updated} users`)
  }
  const handleMigrate = async () => {
    try {
      await addAuthorSlugToPosts()
      toast.success('Migration hoàn tất')
    } catch (err) {
      console.error(err)
      toast.error('Lỗi migration')
    }
  }
  ...
  return (
    <div className='min-h-screen p-8 bg-background'>
      ...
      <Button onClick={addAuthorSlugToPosts}>Chạy migration authorSlug</Button>
    </div>
  )
}

                ...
                {isSearching ? (
                  ...
                  searchResults.map((result) => (
                    <button
                      key={result.id}
                      onClick={() => {
                        console.log('Click item:', result.id, result.title)
                        setSearchQuery('') // Xóa chữ trong input
                        setSearchResults([]) // Xóa kết quả
                        setIsFocused(false) // Ẩn dropdown
                        navigate(`/post/${result.id}`)
                      }}
                      className='w-full text-left px-4 py-3 hover:bg-accent/80 transition-colors flex items-center gap-3 text-sm border-b last:border-b-0'
                    >

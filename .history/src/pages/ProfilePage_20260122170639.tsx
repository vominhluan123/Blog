...
const Profile = () => {
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      // Xử lý slug: chỉ tạo mới nếu displayName thay đổi
      let slug = profileData?.slug
      if (!slug || data.displayName !== profileData?.displayName) {
        slug = await getOrCreateUniqueSlug(data.displayName, user.uid)
        console.log('Slug mới được tạo:', slug) // debug để kiểm tra
      }
      ...
      // Cập nhật Firestore user document (luôn update slug để đồng bộ)
      const userRef = doc(db, USERS_COLLECTION, user.uid)
      await updateDoc(userRef, {
        displayName: data.displayName,
        slug, // ← luôn lưu slug (mới hoặc cũ)
        photoURL,
        updatedAt: new Date()
      })

      // Cập nhật tất cả bài viết của user
      ...
      postsSnapshot.docs.forEach((postDoc) => {
        batch.update(postDoc.ref, {
          ...
          // Nếu bạn muốn lưu slug vào post (tùy chọn, nhưng khuyến khích để sidebar nhất quán)
          // authorSlug: slug,
          updatedAt: new Date()
        })
      })
      ...
      // Cập nhật state local (quan trọng: update cả slug)
      setProfileData((prev) => ({
        ...prev,
        displayName: data.displayName,
        photoURL,
        slug // ← thêm dòng này để state có slug mới
      }))
      ...
      window.location.reload() // reload để sidebar lấy slug mới nếu cần
    } catch (error: any) {

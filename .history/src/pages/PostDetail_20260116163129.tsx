  ...
  useEffect(() => {
    ...
    const fetchRelated = async () => {
      ...
      try {
        ...
        console.log('--- DEBUG RELATED POSTS ---')
        console.log('Current post ID:', post.id)
        console.log('Current tags:', post.tags || 'không có')
        console.log('Current category:', post.category)

        // 1. Thử query theo tag nếu có
        if (post.tags && post.tags.length > 0) {
          const tagsToMatch = post.tags.slice(0, 10)
          console.log('Query TAG với:', tagsToMatch)
          ...
          console.log('TAG query → docs found:', snapTag.size)
          console.log(
            'TAG doc IDs:',
            snapTag.docs.map((d) => d.id)
          )
          ...
          console.log('TAG candidates sau filter:', candidates.length)
        }

        // 2. Nếu tag không có hoặc 0 → fallback category
        if (candidates.length === 0 && post.category) {
          console.log('Fallback CATEGORY:', post.category)

          const qCat = query(postsRef, where('category', '==', post.category), orderBy('createdAt', 'desc'), limit(5))

          const snapCat = await getDocs(qCat)
          console.log('CATEGORY query → docs found:', snapCat.size)
          console.log(
            'CATEGORY doc IDs:',
            snapCat.docs.map((d) => d.id)
          )
          ...
          console.log('CATEGORY candidates sau filter:', candidates.length)
        }

        // 3. Fallback cuối: lấy 4 bài mới nhất KHÔNG điều kiện nào (để test collection có dữ liệu không)
        if (candidates.length === 0) {
          console.log('Fallback TOÀN BỘ bài mới nhất (test collection)')

          const qAll = query(postsRef, orderBy('createdAt', 'desc'), limit(4))

          const snapAll = await getDocs(qAll)
          console.log('ALL query → docs found:', snapAll.size)
          console.log(
            'ALL doc IDs:',
            snapAll.docs.map((d) => d.id)
          )
          ...
          console.log('ALL candidates sau filter:', candidates.length)
        }

        console.log('Final related posts:', candidates.length)

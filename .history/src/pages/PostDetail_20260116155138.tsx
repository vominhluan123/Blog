...
useEffect(() => {
  ...
  const fetchRelated = async () => {
    ...
    try {
      // Lấy tối đa 10 tag để tránh vượt giới hạn array-contains-any
      ...
      const q = query(
        ...
        orderBy('createdAt', 'desc'), // hoặc bỏ orderBy nếu không cần
        limit(6) // lấy dư 1 chút để lọc
      )

      const snapshot = await getDocs(q)
      const candidates = snapshot.docs
        .map((doc) => ({ id: doc.id, ...doc.data() } as Post))
        // Loại bỏ chính bài đang xem
        .filter((p) => p.id !== post.id)

      // Optional: sort theo số tag chung (càng nhiều càng liên quan)
      ...
      // Lấy 2 bài đầu
      setRelatedPosts(sorted.slice(0, 2))
    } catch (err) {
  ...
  return (
    <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12'>
      ...
      <article>
        ...
        {/* Related posts (gợi ý 2 bài liên quan - tạm dùng query sau này) */}
        <div className='mt-16'>
          <h2 className='text-h2 font-bold mb-6'>Bài viết liên quan</h2>
          <div className='grid md:grid-cols-2 gap-6'>
            {/* Tạm dùng sample, sau này query real related */}
              ...
              >
                <div className='p-6'>
                  ...
                  <p className='mt-2 text-sm text-muted-foreground line-clamp-2'>
                    Tóm tắt ngắn gọn cho bài liên quan...
                  </p>
                </div>
              </Link>
            ))}

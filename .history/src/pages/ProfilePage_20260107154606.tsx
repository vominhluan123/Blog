...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const userRef = doc(db, Post, user.uid)
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        formData.append('upload_preset', 'blog_upload_img') // preset của bạn

        const response = await fetch(`https://api.cloudinary.com/v1_1/dhxhwcdo5/image/upload`, {
          method: 'POST',
          body: formData
        })
          ...
          <CardContent className='space-y-8'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  value={profileData?.role === 'admin' ? 'Quản trị viên' : 'Người dùng'}
                  disabled

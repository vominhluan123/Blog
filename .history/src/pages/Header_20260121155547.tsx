...
const Header = () => {
  ...
  const [isFocused, setIsFocused] = useState(false) // người dùng đang click vào ô tìm kiếm không?
  const searchRef = useRef<HTMLDivElement>(null) // dùng để biết "click ra ngoài"
  // Click ra ngoài thì ẩn kết quả tìm kiếm
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (searchRef.current && !searchRef.current.contains(e.target as Node)) {
        // Delay 0 để cho onClick chạy trước khi ẩn
        setTimeout(() => {
          setIsFocused(false)
        }, 0)
      }
    ...
    // Đổi sang 'click' thay vì 'mousedown'
        ...
        <div className='flex items-center gap-4 relative' ref={searchRef}>
          <div className='relative hidden md:block'>
            ...
            <Input
              ...
              onBlur={() => {
                setIsFocused(false)
                // Không reset searchResults ở đây nữa → giữ để lần focus sau hiện nhanh nếu query chưa đổi
              }}
              lang='vi'
              spellCheck={false} // ← tắt gạch chân đỏ
              autoCorrect='off'
              autoCapitalize='off'
              className={cn(searchInputClass, 'pl-11 w-64')}
            />
                ...
                {isSearching ? (
                  ...
                  searchResults.map((result) => (
                    <div
                      ...
                      className='w-full text-left px-4 py-3 hover:bg-accent/80 transition-colors flex items-center gap-3 text-sm border-b last:border-b-0 cursor-pointer'
                    >

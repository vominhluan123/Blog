...
import { DEFAULT_POST_STATUS, POSTS_COLLECTION, PostStatus, USERS_COLLECTION, type Post } from '@/firebase/db'
  ...
  useEffect(() => {
    ...
    const fetchUserSlug = async () => {
      ...
      if (userSnap.exists() && userSnap.data().slug) {
        setUserSlug(userSnap.data().slug)
      }
    }
    fetchUserSlug()
  }, [user])
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const postData = {
        ...
        authorSlug: userSlug || 'user-' + user.uid.slice(0, 8),
      }

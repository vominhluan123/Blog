...
const PostDetail = () => {
  ...
  useEffect(() => {
    ...
    // chỉ get 1 lần khi mount
    getDoc(likeRef)
      .then((snap) => {
        setHasLiked(snap.exists())
        if (snap.exists()) {
          setOptimisticLikes(post?.likesCount ?? 0) // đồng bộ lại nếu cần
        }
      })
      .catch(console.error)

    // optional: lắng nghe realtime nếu nhiều người like cùng lúc
    // return onSnapshot(likeRef, snap => setHasLiked(snap.exists()));

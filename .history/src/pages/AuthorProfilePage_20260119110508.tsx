...
import { POSTS_COLLECTION, USERS_COLLECTION, PostStatus, type Post } from '@/firebase/db'
import { Card, CardContent } from '@/components/ui/card'
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar'

const AuthorProfilePage = () => {
  const { authorId } = useParams<{ authorId: string }>() // giờ là slug, không phải uid
  ...
  useEffect(() => {
    ...
    const fetchAuthorData = async () => {
      try {
        ...
        // 1. Tìm user theo slug
        const userQ = query(collection(db, USERS_COLLECTION), where('slug', '==', authorId), limit(1))
        ...
        const uid = userDoc.id // đây là UID thật

        const authorName = userData.displayName || 'Unknown'
        const photoURL = userData.photoURL

        // 2. Lấy bài viết của user này
        const postsQ = query(
          collection(db, POSTS_COLLECTION),
          where('authorId', '==', uid),
          where('status', '==', PostStatus.APPROVED),
          orderBy('createdAt', 'desc')
          // limit(12) // nếu muốn phân trang thì thêm sau
        )

        const postsSnap = await getDocs(postsQ)
        const authorPosts = postsSnap.docs.map((doc) => ({
          id: doc.id,
          ...doc.data()
        })) as Post[]

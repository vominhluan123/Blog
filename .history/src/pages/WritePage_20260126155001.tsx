...
const schema = z
  .object({
    ...
    content: z
      .string()
      .min(1, 'Nội dung không được để trống') // tạm thời cho phép HTML rỗng ban đầu
      .max(10000, 'Nội dung quá dài (tối đa 10.000 ký tự HTML)'),
  ...
  .superRefine((data, ctx) => {
    ...
    if (plainText.trim().length < 10) {
      ctx.addIssue({
        ...
        type: 'string',
        inclusive: true,
        message: 'Nội dung phải có ít nhất 10 ký tự (không tính định dạng)',
        path: ['content']
      })
    }
  })
